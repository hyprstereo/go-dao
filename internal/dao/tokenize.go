...
func (p Tokenize) Trim(s string) Tokenize {
	p = Tokenize(strings.TrimSpace(strings.ReplaceAll(s, " ", ".")))
	if strings.ContainsAny(string(p), ",.'\"!?") {

		p = Tokenize(r.ReplaceAllString(string(p), " "))
	}
	return p
}
...
func (p Tokenize) replace(replacer func(tag string) (string, bool)) Tokenize {
	type rep struct {
	...
	pi = 0 // Reuse as Tokenize index position
	for _, r := range rs {
		if r.o > 0 {
			seg := p[pi:r.o]
			copy(result[o:], seg)
...
// in Tokenize, or -1 if no wildcard is present.
//
// Behavior is undefined for an invalid Tokenize.
