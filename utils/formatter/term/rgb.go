...
func (r *rgbControl) Random() *rgbControl {
	r.r, r.g, r.b = RandomRGB()
	return r
}

func (r *rgbControl) FromHex(hex string) *rgbControl {
	r.r, r.g, r.b = HexToRGB(hex)

	return r
}

func (rc *rgbControl) ShiftHSL(h, l, s float64, foreground bool) []byte {
	fh, fl, fs := RGBtoHSL(rc.r, rc.g, rc.b)
	fh += h
	fl += l
	fs += s
	rc.sh = h
	rc.sl = l
	rc.ss = s
	r, g, b := HSLtoRGB(fh, fl, fs)

	return GetColor(r, g, b, foreground)
}

func (r *rgbControl) NextShift(foreground bool) []byte {
	r.sh++
	r.sl++
	r.ss++
	return r.ShiftHSL(r.sh, r.sl, r.ss, foreground)
}

func (r *rgbControl) Reset() *rgbControl {
	r.r = 0
	r.g = 0
	r.b = 0
	r.sh = 0
...
func FGColorLookUp(val uint8) []byte {
	return fgTermRGB[val]
}

func BGColorLookUp(val uint8) []byte {
	return bgTermRGB[val]
}
...
func FgShift(in, from string, h, l, s float64) string {
	...
	count := len(buff)
	for c := 0; c < count; c++ {
		val = append(val, rc.ShiftHSL(h*float64(c), s*float64(c), s*float64(c), true)...)
		val = append(val, buff[c])

	}
	val = append(val, reset...)
	return string(val)
}
func BgShift(in, from string, h, l, s float64) string {
	...
	count := len(buff)
	for c := 0; c < count; c++ {
		val = append(val, rc.ShiftHSL(h*float64(c), s*float64(c), s*float64(c), false)...)
		val = append(val, buff[c])
...
func GetColor(r, g, b uint8, foreground bool) []byte {
	clr := color(r, g, b, foreground)
	return append(append(before, clr...), after...)
}
